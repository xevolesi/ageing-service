...
class pSp(nn.Module):
    ...
    def load_weights(self):
        if self.opts.checkpoint_path is not None:
            ...
            if self.opts.start_from_encoded_w_plus:
                ...
                self.pretrained_encoder.load_state_dict(
                    self.__get_keys(ckpt, "pretrained_encoder"), strict=True
                )
    ...
    def __load_latent_avg(self, ckpt, repeat=None):
        if "latent_avg" in ckpt:
            self.latent_avg = ckpt["latent_avg"].to("cpu")  # self.opts.device)
    ...
    def __get_pretrained_psp_encoder(self):
        ...
        encoder = psp_encoders.GradualStyleEncoder(
            50, "ir_se", self.n_styles, opts_encoder
        )
        return encoder
